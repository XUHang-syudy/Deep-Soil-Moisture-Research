...
# === Step 5: SHAP Dependence Analysis ===
...
def plot_lowess_with_ci(x, y, ax, n_boot=1000, ci=95, color_line="red", frac=0.7):
    ...
    ax.fill_between(x_grid, np.percentile(boot_preds, (100-ci)/2, axis=0), np.percentile(boot_preds, 100-(100-ci)/2, axis=0), color=to_rgba(color_line, 0.2), zorder=1, lw=0)
...
# --- Define Combined Subplot Layout ---
fig, axes = plt.subplots(3, 1, figsize=(9, 18), dpi=300)
fig.subplots_adjust(wspace=0.3)

# Update axis limit dictionary keys accordingly
yaxis_limits = {'species_code': (-300, 200), 'PET_mm': (-175, 150), 'MAP_gridded_mm': (-70, 100), 'stand_age_yr': (-600, 400)}
xaxis_limits = {'species_code': (100, 112), 'PET_mm': (975, 1225), 'MAP_gridded_mm': (400, 700), 'stand_age_yr': (4, 26)}
combined_features = ['species_code', 'PET_mm', 'MAP_gridded_mm']
labels = ['(b)', '(e)', '(h)']
...
for idx, feature in enumerate(combined_features):
    ...
    if feature != 'species_code': plot_lowess_with_ci(x_val, shap_val, ax_main, color_line="red")
    else:
...
# === Step 6: Individual Plots for Remaining Features ===
...
for feature in other_features:
    ...
    if feature != 'depth_m':
        scatter = main_ax.scatter(x_val, shap_val, s=10, c=X_df['depth_m'], cmap=bam_map, alpha=0.8, vmin=vmin, vmax=vmax)
    else:
    ...
    main_ax.tick_params(which='both', direction='in')
    ...
    if feature == 'stand_age_yr': main_ax.xaxis.set_major_locator(MaxNLocator(integer=True))
...
print(f"✅ All feature dependence plots for the subset model have been processed.")