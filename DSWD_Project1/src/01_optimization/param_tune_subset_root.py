...
rawdata2_row = np.size(rawdata2, 0)  # Calculate row count of X
rawdata2_col = np.size(rawdata2, 1)  # Calculate column count of X

err = np.empty((0, 20)) # Initialize container for processed data (20 columns)

# Iterate through monitoring sites (ID 1 to 44)
for k in range(1, 45):
    ...
    for j in range(0, site_row):
        if df.iloc[j, 1] > 3.0:
            ...
            site_sum_hang.iloc[0, 2] = df.iloc[4, 6]  # Root dry weight density (RDWD)
            site_sum_hang.iloc[0, 3] = df.iloc[5, 6]  # Fine root length density (FRLD)
            
            # Retrieve site-specific variables from auxiliary data
            row_indices = np.where(rawdata.iloc[:, 0] == k)[0]
            site_sum_hang.iloc[0, 4:20] = rawdata.iloc[row_indices, 1:17].values  # Append other variables
...
# Feature Renaming
# Mapping original column indices to descriptive feature names for the "Subset Full-variable Model"
new_feature_names = {
    1: 'Depth(m)',
...
# Select feature subset including mechanism-driven (root) variables
...
# Initialize 1x3 subplot figure for hyperparameter tuning
...
# ===== Subplot 3: Grid Search for leaf, split, and features =====
...
plt.tight_layout()
plt.savefig(FIGURES_DIR / "figS1c_hyperparam_subset_root.png", dpi=300, bbox_inches='tight')
plt.savefig(FIGURES_DIR / "figS1c_hyperparam_subset_root.svg", format='svg')
plt.show()
